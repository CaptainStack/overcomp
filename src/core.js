...
export const INITIAL_STATE = List([empty_hero(25), empty_hero(26), empty_hero(27), empty_hero(28), empty_hero(29), empty_hero(30)]);

export const add_hero_to_roster = (state, hero) => {
  let empty_index = state.findIndex((roster_hero) => roster_hero.name === "Empty");
  if (empty_index !== -1) {
    return state.set(empty_index, hero);
  } else {
    return state;
  }
}
...
export const remove_hero_from_roster = (state, hero) => {
  ...
  if (hero_index !== -1) {
    return state.set(hero_index, empty_hero(hero_index + 25));
  } else {
    return state;
  }
}
